...
/** Similar to LQFixedExtraTwoPhazesController, but now allows to turn left.
  *
  * @param maxTime
  *   its set to 5 if less (necessary for cars to move)
  * @param leftTurnTime
  *   used on left turns instead of timePerCar
  */
class LeftTurnController(timePerCar: Int, leftTurnTime: Int, maxTime: Int)
    extends TrafficController[Boolean]:
  override def initialTrafficState =
    InitialStates.alsoTurnLeft // add inital red lights for left turns
  ...
  private def generateFlushes(removedVeh: List[Vehicle], time: Int, timelineDirection: Direction) =
  ...
  private def processVehicles(
    ...
    def loop(
      ...
      if lq.hasNextIn(dire) then
        ...
        val timeLeftNext       = Concurrent.set(tleft - timeFor(v), timeLeft, timeline)
  ...
  override def process(lq: LanesQue, phaze: Boolean): (LanesQue, StepResult, Boolean) =
    ...
    val flushes = generateFlushes(removedVeh, timePassed, dire._1)

    (newLQ, StepResult(removedVeh, timePassed, lightsData, crossingsData, flushes), !phaze)
  end process
end LeftTurnController
